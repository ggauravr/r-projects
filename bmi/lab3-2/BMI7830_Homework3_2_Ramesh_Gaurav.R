...
constTOMFile <- "Golub_TOM"
constNetworkConstructionFile <- "Golub_Network_Construction.RData"
constModTraitFile <- "ModTraitCorrelations.csv"
constGeneInfoFile <- "GeneInfo.csv"
constGeneEnrichmentFile <- "GeneEnrichmentData.csv"
constWorkingDirectory <- "~/Documents/projects/git-projects/r-projects/bmi/lab3-2"
...
inputData.original <- as.data.frame(exprs(Golub_Merge))
...
traits <- pData(Golub_Merge)
rownames(traits) <- traits$Samples
traits <- subset(traits, select=c("ALL.AML"))
# for consistency, let's call this column Factors
...
verboseScatterplot(abs(geneModuleMembership[moduleGenes, column]),
                   ...
                   ylab="Gene Significance for Trait(ALL/AML)",
...
sProbeSymbols <- getSYMBOL(sProbes, "hu6800.db")
...
gene.symbols <- getSYMBOL(names(inputData.trans), "hu6800.db")
gene.entrez_ids <- getEG(names(inputData.trans), "hu6800.db")
...
GOEnr <- GOenrichmentAnalysis(moduleColors, geneInfo$entrez_ids, organism="human", nBestP=10)
...
# TOMplot(mttom, bwnet$dendrograms[[1]], bwnet$colors[bwnet$blockGenes[[1]]], main="Network Heatmap, Block 1")
